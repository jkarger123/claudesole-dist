...
- Restart the live bridge: `TMUX_TMPDIR=/tmp /opt/homebrew/bin/tmux kill-session -t t2tbridge` (auto-recreates ~5s).
- Worker deploy: `cd text2tune/text2tune-api && export $(grep -E '^CLOUDFLARE_' ../.env | xargs) && npx wrangler@4 deploy`.
...
1. **Create new key:** console.anthropic.com -> Settings -> API Keys -> Create Key (name it, e.g.,
   `t2t-runtime-2026-06`). Copy it once.
2. **Update the live runtime:** edit `/Volumes/Samsung990PRO/text2tune-runtime/.env`, set
   `ANTHROPIC_API_KEY=<new>`. (Do NOT put it anywhere that gets committed.)
3. **Restart the bridge:** `tmux kill-session -t t2tbridge` (above), then `tail -20 .../bridge.log`.
...
## 2. Bridge secret  (TEXT2TUNE_BRIDGE_SECRET)
...
2. **Set it on the Worker:** `cd text2tune/text2tune-api && npx wrangler@4 secret put TEXT2TUNE_BRIDGE_SECRET`
   (paste the new value when prompted).
3. **Set it in the runtime:** edit `/Volumes/Samsung990PRO/text2tune-runtime/.env`,
   `TEXT2TUNE_BRIDGE_SECRET=<new>`.
...
2. **Update `.env`:** set `CLOUDFLARE_API_TOKEN=<new>` in the `.env` your deploys read
   (`/Volumes/Samsung990PRO/hptuners/.env` and/or `text2tune/.env`).
...
1. **Dry-run find:** `~/hptuners-control/bin/gitleaks git /Volumes/Samsung990PRO/hptuners --redact` to
...
4. **Re-clone** anywhere the repo is mirrored (T490 source-of-truth, etc.).
