...
export default class RFB extends EventTargetMixin {
    ...
    _isSupportedSecurityType(type) {
        ...
        // HPCC patch: on an insecure origin (plain http to a Tailscale IP) the Web Crypto API
        // (crypto.subtle) is unavailable, so auth schemes that need it crash ("Cannot read
        // properties of undefined (reading 'importKey')"). Drop them here so negotiation falls
        // through to VNC-password auth (type 2 = pure-JS DES, no Web Crypto needed).
        if (!window.isSecureContext &&
            (type === securityTypeARD || type === securityTypeRA2ne ||
             type === securityTypeVeNCrypt || type === securityTypePlain)) {
            return false;
        }
