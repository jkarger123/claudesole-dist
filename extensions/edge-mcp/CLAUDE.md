...
- SHIPPED edge-mcp Windows browser-host support as v0.99.203 (CCR ccr-1784223446960 -> shipped). Pushed core (branch edge-mcp-indesign-doctrine 6c5417a) + public mirror (839801a), re-signed core (168 files, sign on overseer :8800 not :8799). Fleet converged 10/11 nodes; t480 registered as a live browser host on the OVERSEER (vault:EDGE_SSH_KEY_T480, node_dir=C:\Users\CBK\.edge-mcp\node for portable Node 22), probe ok=true. CAVEAT: afp stuck on 0.99.202 -- pre-existing cryptography-missing on AFP blocks superadmin verify (see afp-node-ops memory); overseer had same issue, fixed by restart. Neutra
<!-- /CC:NOTES -->
