...
> Repo root: `/Users/hptuner/hptuners-control` (the control plane; the canonical *project* tree lives on the SSD at `/Volumes/Samsung990PRO/hptuners`).
...
**Peer roster (`peers.json` + `peers()`).** Shared instance list `{id,url}`; current fleet: hptuners (8443), mission-control (default 443), carsearch (10000), afp (8850) — all on one Tailscale host (`sarahs-mac-studio-2.taile3722a.ts.net`). `peers()` unions the shared file with a local `_instances.json`, deduped by URL; `INSTANCE_ID` lets broadcasts skip the node's own chief; served at `/api/peers`.
...
| **SSD data** | `data/ -> /Volumes/Samsung990PRO/hptuners-control-data` |
| Deliverables (this node) | `/Volumes/Samsung990PRO/hptuners-control-data/deliverables/hptuners` |
