...
- **Product** (a SaaS app, a marketplace): one product organized into **Modules** (its parts). The Projects lens
  + the module map serve this.
- **Agency** (a services firm, e.g. "Acme Agency"): a services business organized into **Clients, Partners, Pipeline, and a
...
code. It was modeled directly from a real agency's tree so it fits how they already work, and generalized.
...
Anything else top-level (e.g. `Internal`, `Routines`) stays a normal module / existing lens.
...
- **Client** = a folder under `Clients/` (or under `Partners/<p>/clients/`). It contains:
  - **tool-applications** -- subfolders whose name matches a Tool id (e.g. `Clients/acme-client/invoice`
...
Primary signal: the application subfolder's NAME matches a Tool id (robust; matches a real agency tree exactly).
...
- **Zero per-agency code** -- it interprets folders + CLAUDE.md (the same substrate as Product), so an
  agency just structures its tree (Clients/Partners/Pipeline/Tools) and gets the lens. An existing agency tree already
  matches, so it gets the lens on the next `cc-update` with no restructuring.
