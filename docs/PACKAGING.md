...
| P2-12 | 47 tenant-residue hits still ratchet-tolerated (incl. a hardcoded dev-PIN fallback) | ✅ ratcheted **47 → 0** and re-baselined to 0 (`.residue_baseline.json`) -- clean-core is now ENFORCED: any framework file that gains a tenant marker FAILS the ship. Comment/example/test-fixture mentions were genericized; the functional defaults are now config-driven -- service session labels come from cc.config `services` via `_service_labels()` (no hardcoded service names), the cwd-shortening regex is generic (last-two-segments, works on any install), the product-bridge chip label is generic (2026-07-09) |
