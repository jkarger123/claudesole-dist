...
Claude Code. Per-deployment it can be re-branded (this deployment's `brand` is "text2tune"), but the
...
- The project tree itself (e.g. `/Volumes/Samsung990PRO/hptuners`) + its CLAUDE.md hierarchy.
...
```
bash ~/hptuners-control/cc-init.sh <project_root> [project_name] [brand]
```
...
(hptuners) is the dev/master where features are added.
```
bash ~/hptuners-control/cc-update.sh <git-url|local-dir> [--dry-run]
```
...
1. **Frontend still has hardcoded project paths** -- `PROJ()` and the "New session" default cwd in the
   page (~`server.py` PAGE) return `/Volumes/Samsung990PRO/hptuners`. Inject `PROJECT`/`PROJECT_NAME` into
...
2. **Brand/title** -- the HTML `<title>` + the page header should use `BRAND` from config (server already
   reads it; just template it into the PAGE). The brand is currently the literal "text2tune".
3. **Fleet is hptuners-specific** -- `SERVICES` (bridge/crons/brain) + `_machines.json` + `_components.json`
   are this deployment's. Move them into config (or a per-project `fleet.json`).
4. **Framework home** -- everything assumes `~/hptuners-control`. For a clean product, make the framework
